...
CONT_WEIGHT = 5 # help="weight given to contamination for assessing genome quality", type=float, default=2)
SEL_MIN_QUALITY = 20.0 #add_argument('-q', '--sel_min_quality', help="minimum quality of bin to consider during bin selection process", type=float, default=50)
...
ADD_MATCHES = 3 #add_argument('-m', '--add_matches', help="minimum number of matched bins required to 'add' contigs", type=int, default=2)
#这个我目前并没用上
REPORT_MIN_QUALITY = 20.0 #add_argument('--report_min_quality', help="minimum quality of bin to report", type=float, default=10)
#这个是不是应该是50-2*10好些？？？
...
#bacar_dict_out ='/home/wangzy/data/data_for_review/cami_10sample/test_new_ensemble_strategy/gsa_f1k.fa.bacar.seed.seed_contig_dict.pkl'
with open (bacar_dict_out, 'rb') as f: #打开文件
    bacar_seed_contig_dict = pickle.load(f) #将二进制文件对象转换成 Python 对象

#bacteria_dict_out ='/home/wangzy/data/data_for_review/cami_10sample/test_new_ensemble_strategy/gsa_f1k.fa.marker.seed.seed_contig_dict.pkl'
...
while True:
    ...
    matched_sets = ensemble._matched_bin_sets(bins,
                                              ...
                                              SEL_MIN_COMP,
                                              SEL_MAX_CONT,
                                              False,overlap_strategy=overlap_strategy,sort_quality_strategy = sort_quality_strategy,quality_filter=quality_filter)
