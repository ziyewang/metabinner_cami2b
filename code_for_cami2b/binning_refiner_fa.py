...
combined_all_bins_file = '%s/%s/combined_all_bins.fa' % (wd, output_folder)
separator = '__'
for each_folder in input_bin_folder_list:
    ...
    for each_bin in each_folder_bins:
        ...
        new = open('%s/%s/%s_new/%s_%s.fa' % (wd, output_folder, each_folder, each_folder, bin_file_name), 'w')
    ...
    os.system('cat %s/%s/%s_new/*.fa > %s/%s/combined_%s_bins.fa' % (wd, output_folder, each_folder, wd, output_folder, each_folder))
